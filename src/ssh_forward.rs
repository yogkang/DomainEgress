...
impl SshForwardManager {
    ...
    pub fn start(&self, rule: &SshForwardRule) -> Result<u16> {
        self.stop(&rule.id)?;
        if rule.remote_host.trim().is_empty() || rule.remote_port == 0 || rule.ssh_host.trim().is_empty() || rule.ssh_port == 0 { return Err(anyhow!("SSH 转发配置不完整")); }
        ...
        if ssh_username.trim().is_empty() || ssh_port == 0 { return Err(anyhow!("未从 SSH 配置或当前转发项解析出用户名/端口")); }
        let mut command = Command::new("/usr/bin/ssh");
        command.args(["-N", "-T", "-C", "-o", "ServerAliveInterval=60", "-o", "ServerAliveCountMax=3", "-o", "ExitOnForwardFailure=yes"]);
        for option in &rule.ssh_options { if let Some((key, value)) = option.split_once('=') { command.args(["-o", &format!("{key}={value}")]); } }
        command.arg("-L").arg(format!("{}:{}:{}:{}", rule.bind_host, port, rule.remote_host, rule.remote_port));
        if let Some(key) = rule.ssh_keychain_id.as_ref().filter(|key| !key.trim().is_empty()).or(identity_file.as_ref()) { command.args(["-i", key]); }
        command.arg(format!("{}@{}", ssh_username, ssh_host)).arg("-p").arg(ssh_port.to_string()).stdout(Stdio::null()).stderr(Stdio::piped());
...
fn resolve_ssh_config(rule: &SshForwardRule) -> (String, String, u16, Option<String>) {
    let target = if rule.ssh_username.trim().is_empty() { rule.ssh_host.clone() } else { format!("{}@{}", rule.ssh_username, rule.ssh_host) };
    let output = Command::new("/usr/bin/ssh").args(["-G", &target]).output().ok();
    let mut host = rule.ssh_host.clone(); let mut user = rule.ssh_username.clone(); let mut port = rule.ssh_port; let mut identity = None;
    if let Some(output) = output.filter(|output| output.status.success()) { for line in String::from_utf8_lossy(&output.stdout).lines() { let mut parts = line.splitn(2, ' '); match (parts.next(), parts.next()) { (Some("hostname"), Some(value)) => host = value.trim().into(), (Some("user"), Some(value)) => user = value.trim().into(), (Some("port"), Some(value)) => port = value.trim().parse().unwrap_or(port), (Some("identityfile"), Some(value)) if identity.is_none() && value.trim() != "none" => identity = Some(value.trim().into()), _ => {} } } }
