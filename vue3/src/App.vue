...
async function checkUpdate() { if (!desktop) return; updateBusy.value = true; try { updateInfo.value = await call<UpdateInfo>('check_update') } catch (e) { updateInfo.value = { current_version: '0.1.2', latest_version: null, release_url: null, available: false, error: String(e) } } finally { updateBusy.value = false } }
...
function addSshForward() { const id = `forward-${Date.now()}`; config.value.ssh_forwards.push({ id, name: `端口转发 ${config.value.ssh_forwards.length + 1}`, project: '默认项目', note: '', local_port: null, remote_host: '127.0.0.1', remote_port: 3306, bind_host: '127.0.0.1', ssh_host: '', ssh_port: 22, ssh_username: '', ssh_keychain_id: null, auto_start: false }) }
