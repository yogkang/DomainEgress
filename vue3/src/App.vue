...
import { Activity, ArrowUpRight, Check, ChevronLeft, ChevronRight, CircleHelp, CircleStop, Cloud, Copy, Download, Globe2, ListFilter, Network, Play, Plus, RefreshCw, Search, Settings2, ShieldCheck, Trash2, Upload, X } from 'lucide-vue-next'
import { appearance, theme, themes, appearanceError, setAppearance, setTheme } from './theme'
import { call, defaults, desktop, type CloudAccount, type CloudRegion, type CloudSecurityGroup, type CloudSecurityRule, type Config, type CreateManagedRuleInput, type EgressAddress, type LogEntry, type ManagedRuleConfig, type ManagedRuleSyncResult, type NetworkInterface, type PortRow, type PublicIpProbe, type SaveCloudAccountInput, type Snapshot, type SshForwardRule, type SshProfile, type UpdateInfo } from './api'
...
const pageEyebrow = computed(() => ({ overview: 'NETWORK OVERVIEW', rules: 'ACCESS POLICY', logs: 'REQUEST LOGS', ports: 'SYSTEM NETWORK', 'ssh-forward': 'SSH PORT FORWARDING', cloud: 'CLOUD RESOURCES', settings: 'PREFERENCES' }[tab.value] || 'WORKSPACE'))
const pageDescription = computed(() => ({ overview: '让每一次网络访问，都在掌控之中。', rules: '定义允许或拒绝访问的域名与 IP 地址。', logs: '查看本次运行的访问决策与请求信息。', ports: '查看本机 TCP 监听端口及所属进程。', 'ssh-forward': '将远程服务器端口安全映射到本机访问。', cloud: '按云账号管理远程资源与网络访问能力。', settings: '管理代理监听地址、启动行为与日志策略。' }[tab.value] || ''))
...
function navigate(id: string) { tab.value = id; if (id === 'ports' && desktop && !portLoaded.value) void loadPorts(); if (id === 'cloud') void loadCloudAccounts() }
...
onMounted(async () => { window.addEventListener('keydown', handleFontScaleShortcut); seedCloudPreview(); await refresh(true); await checkIcloud(); await loadManagedRules(); void probePublicIp(); void checkUpdate(); if (desktop) { void syncAllManagedRules(true); cloudSyncTimer = window.setInterval(() => void syncAllManagedRules(true), 5 * 60 * 1000) } if (!disposed) timer = setTimeout(poll, 1500) })
