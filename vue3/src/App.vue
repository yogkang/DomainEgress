    ...
    <main>
      ...
      <div class="content">
        ...
        <template v-if="tab === 'overview'">
          ...
          <section class="panel public-ip-panel"><div class="section-heading"><div><h3>外网出口 IP</h3><p>通过两个独立来源探测，只有结果一致时才标记为可信。</p></div><button :disabled="publicIpBusy || !desktop" @click="probePublicIp"><RefreshCw :size="15" :class="{ spin: publicIpBusy }" />{{ publicIpBusy ? '探测中…' : '双源探测' }}</button></div><div class="public-ip-result"><strong v-if="publicIp.ip" class="mono">{{ publicIp.ip }}</strong><span v-else class="muted">{{ publicIp.error || '尚未探测' }}</span><span class="pill" :class="{ green: publicIp.confidence === '双源一致', red: publicIp.error }">{{ publicIp.confidence }}</span></div><p class="footnote">来源：{{ publicIp.sources.length ? publicIp.sources.join('、') : '—' }} · 结果仅供当前网络诊断参考。</p></section>
