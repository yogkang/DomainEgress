    ...
    <main>
      ...
      <div class="content" :class="{ 'forward-view': tab === 'ssh-forward', 'forward-menu-collapsed': forwardMenuCollapsed }">
        ...
        <template v-if="tab === 'settings' || tab === 'ssh-forward'">
          ...
          <section class="panel about"><span class="brand-mark"><Globe2 :size="24" /></span><div><h3>DomainEgress</h3><div class="about-version"><p>版本 0.2.1 · 本地网络安全代理</p><button @click="checkUpdate" :disabled="updateBusy"><RefreshCw :size="14" :class="{ spin: updateBusy }" />{{ updateBusy ? '检查中…' : '检查更新' }}</button></div><p v-if="updateInfo && !updateInfo.available" class="muted">{{ updateInfo.error ? '暂时无法获取更新信息' : `当前已是最新版本 v${updateInfo.current_version}` }}</p><p v-else-if="updateInfo?.available" class="update-hint">发现新版本 v{{ updateInfo.latest_version }}，请查看上方更新提示。</p><p>关闭窗口后驻留托盘，使用托盘菜单重新打开或退出。</p></div></section>
        </template>
      </div>
    </main>
