    ...
    <main>
      ...
      <div class="content">
        ...
        <template v-if="tab === 'settings'">
          ...
          <section class="panel"><div class="section-heading"><div><h3>Gist 规则同步</h3><p>支持 GitHub Gist 和 Gitee 代码片段；Token 仅用于本次请求，不写入配置文件。</p></div></div><div class="form-grid"><label>服务商<select v-model="config.gist_provider"><option value="github">GitHub Gist</option><option value="gitee">Gitee 代码片段</option></select></label><label>Gist ID<input v-model="config.gist_id" placeholder="例如：a1b2c3d4" autocomplete="off"></label><label>文件名<input v-model="config.gist_file_name" placeholder="domain-egress-rules.json" autocomplete="off"></label><label>访问令牌<input v-model="gistToken" type="password" placeholder="推送需要，拉取公开片段可留空" autocomplete="off"></label></div><div class="toolbar"><button class="primary" :disabled="busy || !desktop || !config.gist_id.trim()" @click="pushGist">推送当前规则</button><button :disabled="busy || !desktop || !config.gist_id.trim()" @click="pullGist">拉取并合并规则</button><span class="muted">拉取结果进入草稿，保存配置后才生效</span></div></section>
          <section class="panel about"><span class="brand-mark"><Globe2 :size="24" /></span><div><h3>DomainEgress</h3><p>版本 0.1.1 · 本地网络安全代理</p><p>关闭窗口后驻留托盘，使用托盘菜单重新打开或退出。</p></div></section>
        </template>
      </div>
    </main>
