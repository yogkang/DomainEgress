...
在“应用设置 → Gist 规则同步”中选择 GitHub Gist 或 Gitee 代码片段，填写 Gist ID 和文件名即可拉取或推送规则。推送需要对应平台的访问令牌；令牌只用于当前请求，不会写入配置文件。拉取内容会先合并到草稿，点击“保存配置”后才会生效。
...
- 浏览器预览不能启停代理：预览页面未连接 Rust 核心，请使用打包后的 macOS 应用。
